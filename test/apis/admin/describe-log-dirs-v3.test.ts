...
const { createRequest, parseResponse } = describeLogDirsV3
...
test('parseResponse correctly processes a successful empty response', () => {
  ...
  const response = parseResponse(1, 35, 3, Reader.from(writer))
...
test('parseResponse correctly processes a response with log dirs', () => {
  // Create a successful response with log dirs (no totalBytes/usableBytes in v3)
  const writer = Writer.create()
    .appendInt32(0) // throttleTimeMs
    .appendInt16(0) // errorCode
    .appendArray(
...
test('parseResponse throws ResponseError on top-level error response', () => {
  ...
  throws(
    () => {
      parseResponse(1, 35, 3, Reader.from(writer))
    },
    (err: any) => {
...
test('parseResponse throws ResponseError on result-level error response', () => {
  // Create a response with a result-level error (no totalBytes/usableBytes in v3)
  const writer = Writer.create()
    .appendInt32(0) // throttleTimeMs
