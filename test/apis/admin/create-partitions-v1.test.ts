...
import { createPartitionsV1, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = createPartitionsV1
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const writer = Writer.create()
    ...
    .appendArray(
      ...
      (w, result) => {
        w.appendString(result.name, false).appendInt16(result.errorCode).appendString(result.errorMessage, false)
      },
      false,
      false
    )

  const response = parseResponse(1, 37, 1, Reader.from(writer))
...
test('parseResponse handles error responses', () => {
  ...
  throws(
    () => {
      parseResponse(1, 37, 1, Reader.from(writer))
    },
    (err: any) => {
