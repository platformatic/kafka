...
import { describeGroupsV2, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = describeGroupsV2
...
test('uses classic request and response headers without tags', () => {
  ...
  deepStrictEqual(
    ...
    { key: 15, version: 2, requestTags: false, responseTags: false }
  )
})
...
test('parseResponse normalizes legacy members and unavailable authorized operations', () => {
  ...
  const response = parseResponse(1, 15, 2, reader)
  deepStrictEqual(
...
test('parseResponse processes multiple groups and empty members', () => {
  ...
  const response = parseResponse(1, 15, 2, Reader.from(writer))
  deepStrictEqual(
...
test('parseResponse processes an empty groups response', () => {
  ...
  deepStrictEqual(parseResponse(1, 15, 2, reader), { throttleTimeMs: 0, groups: [] })
  strictEqual(reader.remaining, 0)
})
