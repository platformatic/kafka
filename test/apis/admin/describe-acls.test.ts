...
test('describeAclsV3 handles a request-response interaction', () => {
  ...
  responseWriter.appendArray(
    resources,
    (w, resource) => {
      w.appendInt8(resource.resourceType).appendString(resource.resourceName).appendInt8(resource.patternType)

      // Append acls array for this resource
      w.appendArray(
        resource.acls,
        (w, acl) => {
          w.appendString(acl.principal).appendString(acl.host).appendInt8(acl.operation).appendInt8(acl.permissionType)
        },
        true,
        false
      )
