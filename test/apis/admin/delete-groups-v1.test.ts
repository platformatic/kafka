...
import { deleteGroupsV1, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = deleteGroupsV1
...
test('uses classic request and response headers without tags', () => {
  ...
  deepStrictEqual(
    ...
    { key: 42, version: 1, requestTags: false, responseTags: false }
  )
})
...
test('parseResponse processes successful classic results', () => {
  ...
  deepStrictEqual(parseResponse(1, 42, 1, reader), {
    throttleTimeMs: 0,
    results: [
      { groupId: 'group-1', errorCode: 0 },
      { groupId: 'group-2', errorCode: 0 }
...
test('parseResponse preserves throttle time and an empty classic results array', () => {
  ...
  deepStrictEqual(parseResponse(1, 42, 1, reader), { throttleTimeMs: 100, results: [] })
  strictEqual(reader.remaining, 0)
})
...
test('parseResponse throws a ResponseError and preserves failed classic results', () => {
  ...
  throws(() => parseResponse(1, 42, 1, reader), error => {
