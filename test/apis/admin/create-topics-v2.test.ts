...
import { createTopicsV2, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = createTopicsV2

test('CreateTopics v2 serializes classic requests', () => {
  ...
  deepStrictEqual({ key: api.key, version: api.version }, { key: 19, version: 2 })
})

test('CreateTopics v2 normalizes fields absent from classic responses', () => {
  ...
  deepStrictEqual(parseResponse(1, 19, 2, Reader.from(writer)), {
    throttleTimeMs: 10,
    topics: [
      {
        name: 'topic',
...
test('CreateTopics v2 preserves normalized topics in errors', () => {
  const writer = Writer.create()
    .appendInt32(0)
    .appendArray(
  ...
  throws(
    () => parseResponse(1, 19, 2, Reader.from(writer)),
    error => {
