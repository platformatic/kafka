...
import { leaveGroupV1, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = leaveGroupV1
...
test('parseResponse correctly processes a successful response', () => {
  deepStrictEqual(parseResponse(1, 13, 1, Reader.from(Writer.create().appendInt32(0).appendInt16(0))), {
    throttleTimeMs: 0,
    errorCode: 0,
    members: []
  })
...
test('parseResponse handles throttling', () => {
  deepStrictEqual(parseResponse(1, 13, 1, Reader.from(Writer.create().appendInt32(100).appendInt16(0))), {
    throttleTimeMs: 100,
    errorCode: 0,
    members: []
  })
...
test('parseResponse handles error code', () => {
  throws(() => {
    parseResponse(1, 13, 1, Reader.from(Writer.create().appendInt32(0).appendInt16(16)))
  }, error => {
