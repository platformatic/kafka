...
import { metadataV5, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = metadataV5

test('uses Metadata v5 and serializes classic request parameters', () => {
  ...
  deepStrictEqual({ key: api.key, version: api.version }, { key: 3, version: 5 })
})
...
test('parses a complete classic response and normalizes the leader epoch', () => {
  ...
  deepStrictEqual(parseResponse(1, 3, 5, Reader.from(writer)), {
    throttleTimeMs: 0,
...
test('reports topic and partition errors from classic responses', () => {
  ...
  throws(() => parseResponse(1, 3, 5, Reader.from(writer)), error => {
