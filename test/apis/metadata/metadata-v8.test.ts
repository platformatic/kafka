...
test('createRequest serializes request parameters correctly', () => {
  ...
  const writer = createRequest(topics, allowAutoTopicCreation, includeTopicAuthorizedOperations, includeClusterAuthorizedOperations)
  ...
  const serialized = {
    topics: reader.readArray(r => {
      const topic = r.readString(false) // Read topic name
      return { topic }
    }, false, false),
...
test('createRequest enables auto topic creation by default', () => {
  ...
  reader.readNullableArray(() => '', false)
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const writer = Writer.create()
    ...
    // Brokers array - classic array format
    .appendArray(
      [
        {
          nodeId: 1,
    ...
    .appendString('test-cluster', false) // clusterId - classic string
  ...
  const response = parseResponse(1, 3, 8, Reader.from(writer))
...
test('parseResponse throws error on topic error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 3, 8, Reader.from(writer))
    },
    (err: any) => {
