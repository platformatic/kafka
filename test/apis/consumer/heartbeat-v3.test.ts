...
import { heartbeatV3, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = heartbeatV3
...
test('createRequest serializes basic parameters correctly', () => {
  ...
  deepStrictEqual(data, { groupId, generationId, memberId, groupInstanceId })
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 12, 3, Reader.from(writer))
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(
    () => { parseResponse(1, 12, 3, Reader.from(writer)) },
    (err: any) => {
