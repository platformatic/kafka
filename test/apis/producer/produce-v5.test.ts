...
import { type MessageRecord, ProduceAcks, produceV5, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = produceV5
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 0, 5, Reader.from(writer))

  // Verify structure - v5 always has empty recordErrors and null errorMessage
...
test('parseResponse handles partition error codes', () => {
  ...
  throws(
    () => {
      parseResponse(1, 0, 5, Reader.from(writer))
    },
    (err: any) => {
...
test('parseResponse handles throttling', () => {
  ...
  // Verify response structure with throttling - v5 always has empty recordErrors and null errorMessage
  deepStrictEqual(response, {
    responses: [
      {
        name: 'test-topic',
