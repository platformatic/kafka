...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 0, 7, Reader.from(writer))

  // Verify structure - v7 always has empty recordErrors and null errorMessage
...
test('parseResponse handles partition error codes', () => {
  ...
  throws(
    () => {
      parseResponse(1, 0, 7, Reader.from(writer))
    },
    (err: any) => {
...
test('parseResponse handles throttling', () => {
  ...
  // Verify response structure with throttling - v7 always has empty recordErrors and null errorMessage
  deepStrictEqual(response, {
    responses: [
      {
        name: 'test-topic',
