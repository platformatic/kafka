...
import { addPartitionsToTxnV4, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = addPartitionsToTxnV4
...
test('parseResponse correctly processes a successful simple response', () => {
  ...
  const response = parseResponse(1, 24, 4, Reader.from(writer))
