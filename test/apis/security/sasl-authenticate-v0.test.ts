...
import { Reader, ResponseError, saslAuthenticateV0, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = saslAuthenticateV0
...
test('parseResponse processes a successful response wire fixture', () => {
  const reader = Reader.from(Buffer.from('0000ffff00000005746f6b656e', 'hex'))

  deepStrictEqual(parseResponse(1, 36, 0, reader), {
    errorCode: 0,
    errorMessage: null,
...
test('parseResponse reports errors from the response wire fixture', () => {
  const reader = Reader.from(Buffer.from('003affff00000000', 'hex'))

  throws(() => parseResponse(1, 36, 0, reader), error => {
