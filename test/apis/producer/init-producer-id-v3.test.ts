...
import { initProducerIdV3, Reader, ResponseError, UserError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = initProducerIdV3
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 22, 3, Reader.from(writer))
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 22, 3, Reader.from(writer))
    },
    (err: any) => {
