  ...
  return Writer.create()
    .appendArray(topics, (w, topic) => {
      w.appendString(topic.name)
        ...
        .appendArray(topic.configs, (w, config) => w.appendString(config.name).appendString(config.value))
    })
...
/*
  CreateTopics Response (Version: 6) => throttle_time_ms [topics] TAG_BUFFER
    ...
    topics => name error_code error_message num_partitions replication_factor configs TAG_BUFFER
      ...
      num_partitions => INT32
      replication_factor => INT16
      configs => COMPACT_NULLABLE_ARRAY
        name => COMPACT_STRING
        value => COMPACT_NULLABLE_STRING
        read_only => BOOLEAN
        config_source => INT8
...
export function parseResponse (
  ...
  const response: CreateTopicsResponse = {
    ...
    topics: reader.readArray((r, index) => {
...
export const api = createAPI<CreateTopicsRequest, CreateTopicsResponse>(19, 6, createRequest, parseResponse)
