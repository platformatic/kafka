...
import { metadataV10, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = metadataV10
...
test('createRequest serializes request parameters correctly', () => {
  ...
  deepStrictEqual(
    serialized,
    {
      ...
      includeClusterAuthorizedOperations: false, // Always false in v10 since is not supported in newer versions
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 3, 10, Reader.from(writer))
...
test('parseResponse throws error on topic error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 3, 10, Reader.from(writer))
    },
    (err: any) => {
