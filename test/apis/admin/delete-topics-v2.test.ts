...
import { deleteTopicsV2, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = deleteTopicsV2

test('DeleteTopics v2 serializes classic names and ignores topic IDs', () => {
  ...
  deepStrictEqual({ key: api.key, version: api.version }, { key: 20, version: 2 })
})

test('DeleteTopics v2 serializes an empty classic topic array', () => {
...
test('DeleteTopics v2 parses classic responses with normalized fields', () => {
  ...
  deepStrictEqual(parseResponse(1, 20, 2, Reader.from(writer)), { throttleTimeMs: 25, responses: [{ name: 'topic', topicId: '00000000-0000-0000-0000-000000000000', errorCode: 0, errorMessage: null }] })
})

test('DeleteTopics v2 preserves normalized fields in errors', () => {
  const writer = Writer.create().appendInt32(0).appendArray([{ name: 'topic', errorCode: 3 }], (w, topic) => {
    w.appendString(topic.name, false).appendInt16(topic.errorCode)
  }, false, false)
  throws(() => parseResponse(1, 20, 2, Reader.from(writer)), error => {
