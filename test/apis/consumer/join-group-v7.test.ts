...
import { joinGroupV7, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = joinGroupV7
...
test('createRequest serializes basic parameters correctly', () => {
  ...
  // Verify tagged fields (no reason field in v7)
...
test('parseResponse correctly processes a successful response for a follower', () => {
  ...
  const response = parseResponse(1, 11, 7, Reader.from(writer))
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 11, 7, Reader.from(writer))
    },
    (err: any) => {
