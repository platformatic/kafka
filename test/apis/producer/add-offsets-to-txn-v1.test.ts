...
import { addOffsetsToTxnV1, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = addOffsetsToTxnV1
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 25, 1, Reader.from(writer))
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 25, 1, Reader.from(writer))
    },
    (err: any) => {
