...
| `consumer:heartbeat:stalled` | `ConsumerHeartbeatStalledPayload` | Emitted once per stall episode when no heartbeat has succeeded within `heartbeatStallTimeout`. Heartbeat handling is unchanged after the event; a successful heartbeat re-arms the tracking. |
...
| maxBytes              | `number`                                                     | 50MB                      | Maximum amount of data the brokers should return. The value might not be respected by Kafka.                                                                                                                                                                                                                                                                                                         |
| maxBytesPerPartition  | `number`                                                     |                           | Maximum amount of data the broker should return for each partition in a stream fetch request. If not set, it defaults to `maxBytes`. The value might not be respected by Kafka.                                                                                                                                                                                                                      |
...
| beforeDeserialization | `BeforeDeserializationHook`                                  |                           | Hook function called before deserialization of each message component (key, value, headers).<br/><br/>**Experimental:** Does not follow semver and may change in minor/patch releases.<br/><br/>**Note:** Should not be provided when using a `registry`.                                                                                                                                            |
| registry              | `AbstractSchemaRegistry<Key, Value, HeaderKey, HeaderValue>` |                           | Schema registry instance for automatic deserialization with schema management. See the [Confluent Schema Registry](./confluent-schema-registry.md) guide for details.<br/><br/>**Experimental:** Does not follow semver and may change in minor/patch releases.<br/><br/>**Note:** When provided, do not use `deserializers` or `beforeDeserialization`.                                             |
...
| heartbeatStallTimeout | `number`                                                     | `rebalanceTimeout`        | Amount of time in milliseconds without a successful heartbeat before `consumer:heartbeat:stalled` is emitted once for the stall episode. A rejoin extends the stall window by `rebalanceTimeout` instead of suppressing it, and a failed rejoin restarts the window. Retrying continues after the event and a successful heartbeat re-arms the tracking.                                                |
...
> ⚠️ **Experimental API**
> Confluent Schema Registry support and the `registry`/`beforeDeserialization` integration are experimental.
...
| minBytes       | `number`              | `1`       | Minimum amount of data the brokers should return. The value might not be respected by Kafka.                                                                                                                                                                                          |
| maxBytes       | `number`              | 50MB      | Maximum amount of data the brokers should return. The value might not be respected by Kafka. This is a key throughput and memory tuning parameter: increase it to allow larger fetch responses and reduce fetch round trips, or decrease it to cap response size and memory pressure. |
| maxWaitTime    | `number`              | 5 seconds | Maximum amount of time in milliseconds the broker will wait before sending a response to a fetch request.                                                                                                                                                                             |
| isolationLevel | `number`              | `1`       | Kind of isolation applied to fetch requests. It can be used to only read producers-committed messages.<br/><br/> The valid values are defined in the `FetchIsolationLevels` enumeration.                                                                                              |
...
| isolationLevel | `number`                   | Kind of isolation applied to fetch requests. It can be used to only read producers-committed messages.<br/><br/> The valid values are defined in the `FetchIsolationLevels` enumeration.                                                               |
...
| heartbeatStallTimeout | `number`                     | `rebalanceTimeout`        | Amount of time in milliseconds without a successful heartbeat before `consumer:heartbeat:stalled` is emitted once for the stall episode.                                     |
