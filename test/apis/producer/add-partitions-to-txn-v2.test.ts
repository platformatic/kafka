...
import * as addPartitionsToTxnV2 from '../../../src/apis/producer/add-partitions-to-txn-v2.ts'
import { ResponseError } from '../../../src/errors.ts'
import { Reader } from '../../../src/protocol/reader.ts'
import { Writer } from '../../../src/protocol/writer.ts'

test('AddPartitionsToTxn v2 serializes and normalizes the legacy schema', () => {
  strictEqual(addPartitionsToTxnV2.api.version, 2)
  const reader = Reader.from(
    addPartitionsToTxnV2.createRequest([
      {
        transactionalId: 'tx',
        producerId: 1n,
        producerEpoch: 2,
  ...
  deepStrictEqual(
    addPartitionsToTxnV2.parseResponse(
      1,
      24,
      2,
      Reader.from(
...
test('AddPartitionsToTxn v2 exposes partition protocol errors', () => {
  ...
  throws(() => addPartitionsToTxnV2.parseResponse(1, 24, 2, Reader.from(response)), ResponseError)
})
