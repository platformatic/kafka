...
import { joinGroupV4, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = joinGroupV4
...
test('createRequest serializes basic parameters correctly', () => {
  ...
  deepStrictEqual(
    ...
    { groupId, sessionTimeoutMs, rebalanceTimeoutMs, memberId, protocolType }
  )
...
test('createRequest with group instance ID', () => {
  ...
  reader.readInt32()
  reader.readString(false)
  // v4 retains the API parameter but does not serialize static membership.
  deepStrictEqual(reader.readString(false), 'consumer')
})
...
test('parseResponse correctly processes a successful response for a follower', () => {
  const writer = Writer.create()
    ...
    .appendString('follower-member-id', false)
    .appendArray([], () => {}, false, false)
  deepStrictEqual(parseResponse(1, 11, 4, Reader.from(writer)), {
    throttleTimeMs: 0,
    errorCode: 0,
    generationId: 5,
    protocolName: 'range',
...
test('parseResponse correctly processes a successful response for a leader', () => {
  const writer = Writer.create()
    ...
    .appendArray(
      [
        { memberId: 'leader-member-id', metadata: Buffer.from('leader-metadata') },
        { memberId: 'follower-member-id', metadata: Buffer.from('follower-metadata') }
      ],
      (w, member) => w.appendString(member.memberId, false).appendBytes(member.metadata, false),
      false,
      false
    )
  const response = parseResponse(1, 11, 4, Reader.from(writer))
  deepStrictEqual(response, {
    ...
    members: [
      { memberId: 'leader-member-id', groupInstanceId: null, metadata: Buffer.from('leader-metadata') },
...
test('parseResponse throws error on non-zero error code', () => {
  const writer = Writer.create()
    ...
    .appendArray([], () => {}, false, false)
  throws(
    () => parseResponse(1, 11, 4, Reader.from(writer)),
    (err: unknown) => {
