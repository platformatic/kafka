...
import { createTopicsV6, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = createTopicsV6

test('CreateTopics v6 serializes flexible requests', () => {
  ...
  deepStrictEqual({ key: api.key, version: api.version }, { key: 19, version: 6 })
})

test('CreateTopics v6 parses flexible responses', () => {
  ...
  deepStrictEqual(parseResponse(1, 19, 6, Reader.from(writer)), {
    throttleTimeMs: 10,
    topics: [
      {
        name: 'topic',
...
test('CreateTopics v6 preserves normalized topics in errors', () => {
  const writer = Writer.create()
    .appendInt32(0)
    .appendArray(
  ...
  throws(
    () => parseResponse(1, 19, 6, Reader.from(writer)),
    error => {
