...
import { listOffsetsV2, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = listOffsetsV2
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 2, 2, Reader.from(writer))
...
test('parseResponse handles partition-level error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 2, 2, Reader.from(writer))
    },
    (err: any) => {
