...
import { listOffsetsV3, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = listOffsetsV3
...
test('createRequest serializes basic parameters correctly', () => {
  ...
  deepStrictEqual(topicsArray, [{ topicName: 'test-topic', partitions: [{ partitionIndex: 0, timestamp: -1n }] }])
})
...
test('parseResponse correctly processes a successful response', () => {
  const writer = Writer.create()
    ...
    .appendArray(
      [
        {
          name: 'test-topic',
          partitions: [{ partitionIndex: 0, errorCode: 0, timestamp: 1617234567890n, offset: 100n }]
        }
      ],
      (w, topic) => {
  ...
  const response = parseResponse(1, 2, 3, Reader.from(writer))
  ...
  deepStrictEqual(response, {
    ...
    topics: [
      {
        name: 'test-topic',
        partitions: [{ partitionIndex: 0, errorCode: 0, timestamp: 1617234567890n, offset: 100n, leaderEpoch: -1 }]
      }
    ]
  })
})
...
test('parseResponse handles partition-level error code', () => {
  const writer = Writer.create()
    ...
    .appendArray(
      [
        {
          name: 'test-topic',
          partitions: [{ partitionIndex: 0, errorCode: 1, timestamp: 0n, offset: -1n }]
        }
      ],
      (w, topic) => {
        w.appendString(topic.name, false).appendArray(
  ...
  throws(
    () => {
      parseResponse(1, 2, 3, Reader.from(writer))
    },
    (err: any) => {
