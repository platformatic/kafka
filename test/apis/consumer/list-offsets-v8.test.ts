...
import { listOffsetsV8, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = listOffsetsV8
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 2, 8, Reader.from(writer))
...
test('parseResponse handles partition-level error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 2, 8, Reader.from(writer))
    },
    (err: any) => {
      ...
      ok(err.message.includes('ListOffsets(v8)'))
