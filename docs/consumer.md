...
The complete TypeScript type of the `Consumer` is determined by the `serializers` option.
...
| `consumer:heartbeat:group:heartbeat`       | Emitted during successful heartbeats.  |
| `consumer:heartbeat:group:heartbeat:error` | Emitted during failed heartbeats.      |
...
| autocommit        | `boolean \| number`                                 | `true`                    | If to autocommit consumed messages<br/><br/> If it is `true`, then messages are committed immediately.<br/><br/> If it is a number it specifies how often offsets will be committed. Only the last offset for a topic-partition is committed.<br/><br/>If set to `false`, then each message read from the stream will have a `commit` method which should be used to manually commit offsets. |
| minBytes          | `number`                                            | `1`                       | Minimum amount of data the brokers should return. The value might not be respected from Kafka.                                                                                                                                                                                                                                                                                                |
| maxBytes          | `number`                                            | 10MB                      | Maximum amount of data the brokers should return. The value might not be respected from Kafka.                                                                                                                                                                                                                                                                                                |
...
| isolationLevel    | string                                              | `READ_COMMITTED`          | Kind of isolation apply to fetch requests. It can be used to only read producers-committed messages.<br/><br/> The valid values are be defined in `FetchIsolationLevels` enumeration.                                                                                                                                                                                                         |
| deserializers     | `Deserializers<Key, Value, HeaderKey, HeaderValue>` |                           | Object that specifies which serializers to use.<br/><br/>The object should only contain one or more of the `key`, `value`, `headerKey` and `headerValue` properties.                                                                                                                                                                                                                          |
...
| mode         | `string`                        | `LATEST` | Where to start to fetch new messages.<br/><br/> The valid values are be defined in the `MessagesStreamModes` enumeration (see below).                                                                     |
| fallbackMode | `string`                        | `LATEST` | Where to start to fetch new messages when offsets information are lacking for the consumer group.<br/><br/> The valid values are be defined in the `MessagesStreamFallbackModes` enumeration (see below). |
...
| Property | Type                       | Description                                                                                                            |
| -------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| offsets  | `CommitOptionsPartition[]` | Offset to commit. Each offset is an object containing the `topic`, `partition`, `offset` and `leaderEpoch` properties. |

### `listOffsets(options[, callback])`

Lists available offsets for topics.

The return value is a map where keys are in the form `$topic:$partition` and values are array of offsets (where the position represent the partition).
...
| timestamp      | `bigint`   | Timestamp of the offsets to retrieve.<br/><br/> If it is `-1` it will return the last available offset, if it is `-2` it will return the first available offsets.<br/><br/> These special values are also defined in the `ListOffsetTimestamps` enum. |
| isolationLevel | `string`   | Kind of isolation apply to fetch requests. It can be used to only read producers-committed messages.<br/><br/> The valid values are be defined in `FetchIsolationLevels` enumeration. Default is `READ_COMMITTED`.                                    |
...
Joins (eventually creating it) a consumer group.
