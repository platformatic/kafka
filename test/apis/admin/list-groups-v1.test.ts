...
import { listGroupsV1, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = listGroupsV1
...
test('uses classic request and response headers without tags', () => {
  ...
  deepStrictEqual(
    ...
    { key: 16, version: 1, requestTags: false, responseTags: false }
  )
})
...
test('parseResponse processes groups with classic fields', () => {
  ...
  deepStrictEqual(parseResponse(1, 16, 1, reader), {
    throttleTimeMs: 0,
    errorCode: 0,
    groups: [
...
test('parseResponse processes an empty classic groups array', () => {
  ...
  deepStrictEqual(parseResponse(1, 16, 1, reader), { throttleTimeMs: 0, errorCode: 0, groups: [] })
  strictEqual(reader.remaining, 0)
})
...
test('parseResponse preserves a throttle time', () => {
  ...
  strictEqual(parseResponse(1, 16, 1, reader).throttleTimeMs, 100)
  strictEqual(reader.remaining, 0)
})
...
test('parseResponse throws a ResponseError and preserves an error response', () => {
  ...
  throws(() => parseResponse(1, 16, 1, reader), error => {
