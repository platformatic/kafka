...
> {
  ...
  [kGetFetchNode] (metadata: ClusterMetadata, topic: string, partition: number, now: number): number {
    ...
    if (
      metadata.brokers.has(preferredReadReplica.node) &&
  ...
  #fetchNodeForRequest (
    ...
    let requestNode: number | undefined
  ...
  */
        ...
        if (abortedTransactions.length) {
          ...
          for (const batch of partition.records!) {
            if (batch.attributes & IS_CONTROL && batch.records[0].key?.readInt16BE(2) === 0) {
              const abortedRange = abortedRanges.get(batch.producerId)
              if (abortedRange) {
                abortedRange[1] = batch.firstOffset
              }
            }
          }
  ...
  #getRejoinError (error: Error): ProtocolError | null {
    ...
    // A cooperative member which lost its membership or generation no longer owns anything:
    // clear the assignments so the next JoinGroup advertises an empty ownedPartitions set
    // instead of re-claiming partitions the group has already reassigned elsewhere.
