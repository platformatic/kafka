...
import { joinGroupV2, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = joinGroupV2
...
test('createRequest with group instance ID', () => {
  ...
  reader.readInt32()
  reader.readString(false)
  // v2 retains the API parameter but does not serialize static membership.
...
test('parseResponse correctly processes a successful response for a follower', () => {
  ...
  deepStrictEqual(parseResponse(1, 11, 2, Reader.from(writer)), {
    throttleTimeMs: 0,
    errorCode: 0,
    generationId: 5,
    protocolName: 'range',
...
test('parseResponse correctly processes a successful response for a leader', () => {
  ...
  const response = parseResponse(1, 11, 2, Reader.from(writer))
...
test('parseResponse throws error on non-zero error code', () => {
  const writer = Writer.create()
    ...
    .appendArray([], () => {}, false, false)
  throws(
    () => parseResponse(1, 11, 2, Reader.from(writer)),
    (err: unknown) => {
