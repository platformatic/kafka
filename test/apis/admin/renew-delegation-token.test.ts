...
import { renewDelegationTokenV2 } from '../../../src/apis/admin/renew-delegation-token.ts'
...
test('renewDelegationTokenV2 has valid handlers', () => {
  const { createRequest, parseResponse } = captureApiHandlers(renewDelegationTokenV2)
...
test('createRequest serializes parameters correctly', () => {
  ...
  // Read the renew period
  const readRenewPeriod = reader.readInt64()
  deepStrictEqual(readRenewPeriod, 3600000n)
})

test('createRequest handles different hmac values and renew periods', () => {
  const testCases = [
    {
    ...
    },
    {
      ...
      renewPeriodMs: 86400000n // 24 hours
    },
    {
  ...
  for (const testCase of testCases) {
    ...
    const writer = Writer.create()
      ...
      .appendInt64(testCase.renewPeriodMs)
      .appendTaggedFields()
    ...
    const readRenewPeriod = reader.readInt64()
    deepStrictEqual(readRenewPeriod, testCase.renewPeriodMs, 'Renew period should match')
  }
})

test('parseResponse handles successful response', () => {
  const { parseResponse } = captureApiHandlers(renewDelegationTokenV2)
  ...
  // Parse the response
  const response = parseResponse(1, 39, 2, writer.bufferList)
...
test('parseResponse throws on error response', () => {
  ...
  let error: any
  try {
    parseResponse(1, 39, 2, writer.bufferList)
...
test('API mock simulation with callback', () => {
  ...
  renewDelegationTokenV2(mockConnection as any, hmac, renewPeriodMs, (error, result) => {
    callbackCalled = true
