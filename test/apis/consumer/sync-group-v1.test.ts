...
import { Reader, ResponseError, syncGroupV1, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = syncGroupV1
...
test('uses classic request and response headers without tags', () => {
  ...
  deepStrictEqual({ key: api.key, version: api.version, requestTags: sent[4], responseTags: sent[5] }, {
    key: 14, version: 1, requestTags: false, responseTags: false
  })
})
...
test('parseResponse correctly processes a successful response', () => {
  ...
  deepStrictEqual(parseResponse(1, 14, 1, reader), {
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(() => parseResponse(1, 14, 1, reader), error => {
