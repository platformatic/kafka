...
test('createRequest serializes basic parameters correctly', () => {
  ...
  // Read durationFilter
  const serializedDurationFilter = reader.readInt64()
  ...
  deepStrictEqual(serializedDurationFilter, BigInt(0), 'Duration filter should be serialized correctly')
})
...
test('createRequest serializes duration filter correctly', () => {
  const producerIdFilters: bigint[] = []
  ...
  deepStrictEqual(serializedDurationFilter, BigInt(60000), 'Duration filter should be serialized correctly')
})
...
test('parseResponse correctly processes a successful empty response', () => {
  ...
  const response = parseResponse(1, 66, 1, Reader.from(writer))
...
test('parseResponse throws ResponseError on error response', () => {
  ...
  throws(
    () => {
      parseResponse(1, 66, 1, Reader.from(writer))
    },
    (err: any) => {
