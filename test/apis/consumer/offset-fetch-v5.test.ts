...
import { offsetFetchV5, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = offsetFetchV5
...
test('createRequest serializes basic parameters using the v5 wire format', () => {
...
test('createRequest ignores requireStable in v5', () => {
...
test('parseResponse correctly processes a successful response', () => {
  const response = parseResponse(1, 9, 5, Reader.from(writeResponse([
    {
      name: 'test-topic',
...
test('parseResponse handles a top-level error code', () => {
  throws(() => parseResponse(1, 9, 5, Reader.from(writeResponse([], 16))), error => {
...
test('parseResponse handles a partition-level error code', () => {
  throws(() => parseResponse(1, 9, 5, Reader.from(writeResponse([
    {
      name: 'test-topic',
