...
import { addOffsetsToTxnV3, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = addOffsetsToTxnV3
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 25, api.version, Reader.from(writer))
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 25, api.version, Reader.from(writer))
    },
    (err: any) => {
