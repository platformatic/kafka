...
export function createRequest (groupId: string, members: LeaveGroupRequestMember[]): Writer {
  return Writer.create().appendString(groupId, false).appendString(members[0]?.memberId, false)
}
/*
...
export function parseResponse (
  ...
  const response = { throttleTimeMs: 0, errorCode: reader.readInt16(), members: [] }
  if (response.errorCode !== 0) { throw new ResponseError(apiKey, apiVersion, { '/': [response.errorCode, null] }, response) }
  return response
}
export const api = createAPI<LeaveGroupRequest, LeaveGroupResponse>(13, 0, createRequest, parseResponse, false, false)
