...
import { listOffsetsV5, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = listOffsetsV5
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 2, 5, Reader.from(writer))
...
test('parseResponse handles partition-level error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 2, 5, Reader.from(writer))
    },
    (err: any) => {
