...
import { offsetFetchV6, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = offsetFetchV6
...
test('createRequest serializes basic parameters and nested tagged fields using the v6 wire format', () => {
...
test('createRequest ignores requireStable in v6', () => {
...
test('createRequest downconverts to the first group and defaults an empty group', () => {
  ...
  deepStrictEqual(createRequest([], false).buffer, Buffer.from('010000', 'hex'))
})
...
test('parseResponse processes a successful response and automatic nested tag buffers', () => {
  ...
  const response = parseResponse(1, 9, 6, reader)
...
test('parseResponse handles a top-level error code', () => {
  throws(() => parseResponse(1, 9, 6, Reader.from(writeResponse([], 16))), error => {
...
test('parseResponse handles a partition-level error code', () => {
  throws(() => parseResponse(1, 9, 6, Reader.from(writeResponse([
    {
      name: 'test-topic',
