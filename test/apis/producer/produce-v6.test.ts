...
import { type MessageRecord, ProduceAcks, produceV6, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = produceV6
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 0, 6, Reader.from(writer))

  // Verify structure - v6 always has empty recordErrors and null errorMessage
...
test('parseResponse handles partition error codes', () => {
  ...
  throws(
    () => {
      parseResponse(1, 0, 6, Reader.from(writer))
    },
    (err: any) => {
...
test('parseResponse handles throttling', () => {
  ...
  // Verify response structure with throttling - v6 always has empty recordErrors and null errorMessage
  deepStrictEqual(response, {
    responses: [
      {
        name: 'test-topic',
