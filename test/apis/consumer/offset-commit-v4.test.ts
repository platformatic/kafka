...
import { offsetCommitV4, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = offsetCommitV4
...
test('createRequest serializes basic parameters with the legacy retention time', () => {
  ...
  strictEqual(reader.readInt64(), -1n)
  deepStrictEqual(
    reader.readArray(
      r => ({
...
test('parseResponse processes a successful response', () => {
  deepStrictEqual(parseResponse(1, 8, 4, Reader.from(response())), {
    throttleTimeMs: 0,
...
test('parseResponse processes multiple topics and partitions', () => {
  ...
  deepStrictEqual(parseResponse(1, 8, 4, Reader.from(response(topics))), { throttleTimeMs: 0, topics })
})
