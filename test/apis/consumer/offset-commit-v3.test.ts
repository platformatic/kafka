...
import { offsetCommitV3, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = offsetCommitV3
...
test('parseResponse processes a successful response', () => {
  deepStrictEqual(parseResponse(1, 8, 3, Reader.from(response())), {
    throttleTimeMs: 0,
...
test('parseResponse processes multiple topics and partitions', () => {
  ...
  deepStrictEqual(parseResponse(1, 8, 3, Reader.from(response(topics))), { throttleTimeMs: 0, topics })
})
