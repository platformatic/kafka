...
import { metadataV6, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = metadataV6

test('uses Metadata v6 and serializes classic request parameters', () => {
  ...
  deepStrictEqual({ key: api.key, version: api.version }, { key: 3, version: 6 })
})
...
test('parses a complete classic response and normalizes the leader epoch', () => {
  const writer = Writer.create()
    ...
    .appendArray([{ errorCode: 0, name: 'topic-1', isInternal: false }], (w, topic) => {
      w.appendInt16(topic.errorCode)
        ...
        .appendArray([{ errorCode: 0, partitionIndex: 0, leaderId: 1 }], (w, partition) => {
  ...
  deepStrictEqual(parseResponse(1, 3, 6, Reader.from(writer)), {
    throttleTimeMs: 0,
    ...
    topics: [{
      ...
      partitions: [{ errorCode: 0, partitionIndex: 0, leaderId: 1, leaderEpoch: -1, replicaNodes: [1, 2], isrNodes: [1], offlineReplicas: [] }]
    }]
  })
})
...
test('reports topic and partition errors from classic responses', () => {
  const writer = Writer.create()
    ...
    .appendArray([{ errorCode: 3, name: 'missing', isInternal: false }], (w, topic) => {
      w.appendInt16(topic.errorCode)
        .appendString(topic.name, false)
        .appendBoolean(topic.isInternal)
        .appendArray([{ errorCode: 9, partitionIndex: 0, leaderId: -1 }], (w, partition) => {
          w.appendInt16(partition.errorCode)
            .appendInt32(partition.partitionIndex)
            .appendInt32(partition.leaderId)
            .appendArray([], () => {}, false, false)
  ...
  throws(() => parseResponse(1, 3, 6, Reader.from(writer)), error => {
