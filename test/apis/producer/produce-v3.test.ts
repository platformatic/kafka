...
import { type MessageRecord, ProduceAcks, produceV3, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = produceV3

test('Produce v3 creates a non-flexible request with Record Batch payloads', () => {
...
test('Produce v3 marks no-ack requests as having no response', () => {
...
test('Produce v3 adapts successful responses to the current producer response API', () => {
  ...
  deepStrictEqual(parseResponse(1, 0, 3, Reader.from(writer)), {
    responses: [
      {
        name: 'orders',
        partitionResponses: [
...
test('Produce v3 reports partition errors while retaining the adapted response', () => {
  const writer = Writer.create()
    .appendArray(
      [{ name: 'orders', index: 2 }],
      (w, topic) => {
  ...
  throws(
    () => parseResponse(1, 0, 3, Reader.from(writer)),
    (error: unknown) => {
