...
import { addOffsetsToTxnV2, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = addOffsetsToTxnV2
...
test('createRequest serializes parameters correctly', () => {
  ...
  // Read the serialized data to verify correctness (non-flexible: readString(false))
...
test('parseResponse correctly processes a successful response', () => {
  // Create a successful response (non-flexible: no tagged fields)
  const writer = Writer.create()
    .appendInt32(0) // throttleTimeMs
    .appendInt16(0) // errorCode (success)

  const response = parseResponse(1, 25, 2, Reader.from(writer))
...
test('parseResponse handles response with throttling', () => {
  // Create a response with throttling (non-flexible: no tagged fields)
  const writer = Writer.create()
...
test('parseResponse throws error on non-zero error code', () => {
  // Create a response with error (non-flexible: no tagged fields)
  const writer = Writer.create()
    .appendInt32(0) // throttleTimeMs
  ...
  throws(
    () => {
      parseResponse(1, 25, 2, Reader.from(writer))
    },
    (err: any) => {
