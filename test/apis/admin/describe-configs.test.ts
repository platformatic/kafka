...
test('createRequest serializes basic parameters correctly', () => {
  ...
  const resourcesArray = reader.readArray(() => {
    ...
    const configurationKeys = reader.readArray(() => reader.readString(), true, false)
...
test('parseResponse correctly processes a successful empty response', () => {
  ...
  const response = parseResponse(1, 32, 4, Reader.from(writer))
...
test('parseResponse throws ResponseError on error response', () => {
  ...
  throws(
    () => {
      parseResponse(1, 32, 4, Reader.from(writer))
    },
    (err: any) => {
