...
import { offsetFetchV4, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = offsetFetchV4
...
test('createRequest serializes basic parameters using the v4 wire format', () => {
...
test('createRequest ignores requireStable in v4', () => {
...
test('parseResponse correctly processes a successful response', () => {
  const response = parseResponse(1, 9, 4, Reader.from(writeResponse([
...
test('parseResponse handles a top-level error code', () => {
  throws(() => parseResponse(1, 9, 4, Reader.from(writeResponse([], 16))), error => {
...
test('parseResponse handles a partition-level error code', () => {
  throws(() => parseResponse(1, 9, 4, Reader.from(writeResponse([
