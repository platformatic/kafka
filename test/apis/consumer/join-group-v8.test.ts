...
import { joinGroupV8, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = joinGroupV8
...
test('parseResponse correctly processes a successful response for a follower', () => {
  ...
  const response = parseResponse(1, 11, 8, Reader.from(writer))
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 11, 8, Reader.from(writer))
    },
    (err: any) => {
