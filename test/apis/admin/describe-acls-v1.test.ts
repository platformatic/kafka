...
import { api, createRequest, parseResponse } from '../../../src/apis/admin/describe-acls-v1.ts'
import { Reader } from '../../../src/protocol/reader.ts'
import { Writer } from '../../../src/protocol/writer.ts'

test('DescribeAcls v1 handles classic requests, responses, errors, and headers', () => {
  ...
  strictEqual(request.readInt8(), 3)
  strictEqual(request.remaining > 0, true)
  ...
  const sent: unknown[][] = []
  api({ send: (...args: unknown[]) => sent.push(args) } as never, { resourceType: 2, resourceName: null, resourcePatternType: 1, principal: null, host: null, operation: 1, permissionType: 1 })
  deepStrictEqual(sent[0].slice(0, 2).concat(sent[0].slice(4, 6)), [29, 1, false, false])
})
