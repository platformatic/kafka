...
| `clientRack`         | `string`               |           | Client rack identifier used as Kafka `client.rack`; consumers send it as the Fetch and ConsumerGroupHeartbeat rack ID. The `client.rack` option name is also accepted. |
