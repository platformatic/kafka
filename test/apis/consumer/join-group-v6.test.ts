...
import { joinGroupV6, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = joinGroupV6
...
test('parseResponse correctly processes a successful response for a follower', () => {
  ...
  // v6 response has no protocolType field
  ...
  const response = parseResponse(1, 11, 6, Reader.from(writer))

  // Verify structure (no protocolType in v6)
...
test('parseResponse correctly processes a successful response for a leader', () => {
  ...
  // Verify response structure (no protocolType in v6)
  deepStrictEqual(response, {
    throttleTimeMs: 0,
    errorCode: 0,
    generationId: 5,
...
test('parseResponse throws error on non-zero error code', () => {
  // Create a response with error (no protocolType in v6)
  const writer = Writer.create()
    .appendInt32(0) // throttleTimeMs
  ...
  throws(
    () => {
      parseResponse(1, 11, 6, Reader.from(writer))
    },
    (err: any) => {
      ...
      // Verify that the response structure is preserved (no protocolType in v6)
