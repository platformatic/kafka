...
import { heartbeatV2, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { api, createRequest, parseResponse } = heartbeatV2
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 12, 2, Reader.from(writer))
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(
    () => { parseResponse(1, 12, 2, Reader.from(writer)) },
    (err: any) => {
