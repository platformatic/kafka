...
import { apiVersionsV2, protocolAPIsById, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = apiVersionsV2
...
test('parseResponse correctly processes a successful response', () => {
  ...
  deepStrictEqual(parseResponse(1, 18, 2, Reader.from(writer)), {
    errorCode: 0,
    apiKeys: [
...
test('parseResponse throws error on non-zero error code', () => {
  ...
  throws(
    () => parseResponse(1, 18, 2, Reader.from(writer)),
    error => {
