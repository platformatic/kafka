...
import { createRecordsBatch, crc32c, fetchV8, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = fetchV8
...
test('parseResponse correctly processes a successful simple response', () => {
  ...
  const response = parseResponse(1, 1, 8, Reader.from(writer))
...
test('parseResponse handles top-level error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 1, 8, Reader.from(writer))
    },
    (err: any) => {
