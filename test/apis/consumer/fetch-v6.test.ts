...
import { createRecordsBatch, crc32c, fetchV6, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = fetchV6
...
test('createRequest handles forgotten topics data', () => {
  ...
  // Fetch v6 has no forgotten-topics or rack fields.
...
test('parseResponse correctly processes a successful simple response', () => {
  ...
  const response = parseResponse(1, 1, 6, Reader.from(writer))
...
test('parseResponse handles top-level error code', () => {
  // Fetch v6 has no top-level error or session fields.
  const response = parseResponse(1, 1, 6, Reader.from(Writer.create().appendInt32(0).appendArray([], () => {}, false, false)))
...
test('parseResponse handles partition-level error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 1, 6, Reader.from(writer))
    },
    (err: any) => {
...
test('parseResponse handles aborted transactions', () => {
  // Empty batches have a 49-byte body after the length field.
  ...
  emptyRecordsBatch
    .appendUnsignedInt32(crc32c(emptyRecordsBatch.dynamicBuffer), false)
    .appendInt8(2, false) // magic (record format version)
    .appendInt32(0, false) // partitionLeaderEpoch
    .prependLength()
    .appendInt64(0n, false) // firstOffset
...
test('parseResponse handles truncated records', () => {
  ...
  const recordsBatch = Writer.create()
    .appendFrom(completeRecordsBatch)
