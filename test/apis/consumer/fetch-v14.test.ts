...
import { createRecordsBatch, crc32c, fetchV14, Reader, ResponseError, Writer } from '../../../src/index.ts'

const { createRequest, parseResponse } = fetchV14
...
test('parseResponse correctly processes a successful simple response', () => {
  ...
  const response = parseResponse(1, 1, 14, Reader.from(writer))
...
test('parseResponse handles top-level error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 1, 14, Reader.from(writer))
    },
    (err: any) => {
