...
export function writeUnsignedVarInt (buffer: Buffer, offset: number, value: number): number {
  let position = offset
...
export function writeVarInt (buffer: Buffer, offset: number, value: number): number {
  return writeUnsignedVarInt(buffer, offset, intZigZagEncode(value))
}
